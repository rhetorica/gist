...
#include "util_lib.h"
                         ...
                         // observed sequence
void viterbi(HMM *hmm_ptr, char *O, char** results, int whole_genome, int format) {
	...
	char dna_f1[300000];
	char protein[100000];
	char *protein2 = new char[7000000]; protein2[0] = '\0'; // you read that correctly, folks
	int dna_id = 0;
	int dna_f_id = 0;
	int out_nt;
	int start_t;
	...
	for (t = 1; t < len_seq; t++) {
			...
			if (alpha[i][t] < max_dbl) {
				...
				if (t == 0) {
							...
							for (j = M5_STATE; j >= M1_STATE; j--) {
								...
								if (num_d > 0) {
									temp_alpha = alpha[j][t - 1]
											- log(hmm_ptr->tr[TR_MD])
											- log(hmm_ptr->e_M[0][from2][to])
											- log(0.25) * (num_d - 1)
											- log(hmm_ptr->tr[TR_DD])
													* (num_d - 2)
					...
					if (t < 2) {
							...
							// && (t < len_seq) // added by #srw
							// && (temp_i[j - I1_STATE] - 1 >= 0) // added by #srw
							// above changes are untested but apparently unnecessary
							// despite how angry valgrind is
							&& (O[temp_i[j - I1_STATE] - 1] == 'T'
									|| O[temp_i[j - I1_STATE] - 1] == 't')
							&& (((O[temp_i[j - I1_STATE]] == 'A'
									|| O[temp_i[j - I1_STATE]] == 'a')
									&& (O[t] == 'A' || O[t] == 'a'))
									|| ((O[temp_i[j - I1_STATE]] == 'A'
											|| O[temp_i[j - I1_STATE]] == 'a')
											&& (O[t] == 'G' || O[t] == 'g'))
									|| ((O[temp_i[j - I1_STATE]] == 'G'
											|| O[temp_i[j - I1_STATE]] == 'g')
											&& (O[t] == 'A' || O[t] == 'a')))) {
					} else {
						temp_alpha = alpha[j][t - 1] - log(hmm_ptr->tr[TR_IM])
								- log(0.25);
						if (temp_alpha < alpha[i][t]) {
							alpha[i][t] = temp_alpha;
							path[i][t] = j;
			...
			if (t == 0) {
				...
				if (i == I6_STATE) {
					temp_alpha = alpha[j][t - 1] - log(hmm_ptr->tr[TR_GG])
							- log(hmm_ptr->tr[TR_MI])
							- log(hmm_ptr->tr_M_I[from][to]);
				} else {
					temp_alpha = alpha[j][t - 1] - log(hmm_ptr->tr[TR_MI])
							- log(hmm_ptr->tr_M_I[from][to]);
				}
				if (temp_alpha < alpha[i][t]) {
					alpha[i][t] = temp_alpha;
			...
			if ((i == M1_STATE_1 || i == M4_STATE_1) && t >= 3
				...
				if (t == 0) {
					...
					if (i == M1_STATE_1) {
						...
						j = i - 1;
						alpha[i][t] =
								alpha[j][t - 1] - log(hmm_ptr->tr[TR_MM])
										- log(
		...
		for (i = I1_STATE_1; i <= I6_STATE_1; i++) {
			...
			if (t == 0) {
				...
				/* from M state */
				if (path[S_STATE_1][t - 3] != R_STATE
						&& path[S_STATE_1][t - 4] != R_STATE
		...
		if (alpha[E_STATE][t] == 0) {
			...
			if (t < len_seq - 2 && (O[t] == 'T' || O[t] == 't')
				...
				for (i = -60; i <= -3; i++) {
					start_freq -= log(
							hmm_ptr->tr_E[i + 60][trinucleotide(O[t + i],
									O[t + i + 1], O[t + i + 2])]);
				}
				h_kd = hmm_ptr->E_dist[2]
						* exp(
								-1 * pow(start_freq - hmm_ptr->E_dist[1], 2)
										/ (2 * pow(hmm_ptr->E_dist[0], 2)));
				r_kd = hmm_ptr->E_dist[5]
						* exp(
								-1 * pow(start_freq - hmm_ptr->E_dist[4], 2)
										/ (2 * pow(hmm_ptr->E_dist[3], 2)));
				p_kd = h_kd / (h_kd + r_kd);
				if (p_kd < 0.01) {
					p_kd = 0.01;
				} else if (p_kd > 0.99) {
		...
		if (alpha[S_STATE_1][t] == 0) {
			...
			if (t < len_seq - 2 && (O[t + 2] == 'A' || O[t + 2] == 'a')
				...
				alpha[S_STATE_1][t + 2] = alpha[R_STATE][t - 1]
						- log(hmm_ptr->tr[TR_RS]);
				...
				if ((O[t] == 'T' || O[t] == 't')
						&& (O[t + 1] == 'T' || O[t + 1] == 't')) {
					alpha[S_STATE_1][t + 2] = alpha[S_STATE_1][t + 2]
							- log(0.54);
				} else if ((O[t] == 'C' || O[t] == 'c')
						&& (O[t + 1] == 'T' || O[t + 1] == 't')) {
					alpha[S_STATE_1][t + 2] = alpha[S_STATE_1][t + 2]
				...
				for (i = 3; i <= 60; i++) {
					start_freq -= log(
							hmm_ptr->tr_S_1[i - 3][trinucleotide(O[t + i],
									O[t + i + 1], O[t + i + 2])]);
				}
				h_kd = hmm_ptr->S1_dist[2]
						* exp(
								-1 * pow(start_freq - hmm_ptr->S1_dist[1], 2)
										/ (2 * pow(hmm_ptr->S1_dist[0], 2)));
				r_kd = hmm_ptr->S1_dist[5]
						* exp(
								-1 * pow(start_freq - hmm_ptr->S1_dist[4], 2)
										/ (2 * pow(hmm_ptr->S1_dist[3], 2)));
				p_kd = h_kd / (h_kd + r_kd);
				if (p_kd < 0.01) {
					p_kd = 0.01;
				} else if (p_kd > 0.99) {
		...
		if (alpha[S_STATE][t] == 0) {
			...
			if (t < len_seq - 2 && (O[t + 1] == 'T' || O[t + 1] == 't')
					&& (O[t + 2] == 'G' || O[t + 2] == 'g')
					&& ((O[t] == 'A' || O[t] == 'a')
							|| (O[t] == 'G' || O[t] == 'g')
							|| (O[t] == 'T' || O[t] == 't'))) {
				...
				alpha[S_STATE][t + 2] = alpha[R_STATE][t - 1]
						- log(hmm_ptr->tr[TR_RS]);
				...
				for (i = -30; i <= 30; i++) {
					start_freq -= log(
							hmm_ptr->tr_S[i + 30][trinucleotide(O[t + i],
									O[t + i + 1], O[t + i + 2])]);
				}
				h_kd = hmm_ptr->S_dist[2]
						* exp(
								-1 * pow(start_freq - hmm_ptr->S_dist[1], 2)
										/ (2 * pow(hmm_ptr->S_dist[0], 2)));
				r_kd = hmm_ptr->S_dist[5]
						* exp(
								-1 * pow(start_freq - hmm_ptr->S_dist[4], 2)
										/ (2 * pow(hmm_ptr->S_dist[3], 2)));
				p_kd = h_kd / (h_kd + r_kd);
				if (p_kd < 0.01) {
					p_kd = 0.01;
				} else if (p_kd > 0.99) {
		...
		if (alpha[E_STATE_1][t] == 0) {
			...
			if (t < len_seq - 2 && (O[t] == 'C' || O[t] == 'c')
				...
				alpha[E_STATE_1][t + 2] = alpha[M6_STATE_1][t - 1]
						- log(hmm_ptr->tr[TR_GE]);
				...
				if ((O[t + 2] == 'T' || O[t + 2] == 't')) {
					alpha[E_STATE_1][t + 2] = alpha[E_STATE_1][t + 2]
							- log(0.83);
				...
				for (i = -30; i <= 30; i++) {
					start_freq -= log(
							hmm_ptr->tr_E_1[i + 30][trinucleotide(O[t + i],
									O[t + i + 1], O[t + i + 2])]);
				}
				h_kd = hmm_ptr->E1_dist[2]
						* exp(
								-1 * pow(start_freq - hmm_ptr->E1_dist[1], 2)
										/ (2 * pow(hmm_ptr->E1_dist[0], 2)));
				r_kd = hmm_ptr->E1_dist[5]
						* exp(
								-1 * pow(start_freq - hmm_ptr->E1_dist[4], 2)
										/ (2 * pow(hmm_ptr->E1_dist[3], 2)));
				p_kd = h_kd / (h_kd + r_kd);
	...
	for (t = 0; t < len_seq; t++) {
		...
		if (codon_start == 0
			...
			if (vpath[t] < M6_STATE) {
				...
				&& (vpath[t] == E_STATE || vpath[t] == E_STATE_1
						|| t == len_seq - 1)) {
			...
			final_score = (alpha[vpath[end_t - 4]][end_t - 4]
					- alpha[vpath[start_t + 2]][start_t + 2])
					/ (end_t - start_t - 5);
			frame = start_orf % 3;
			if (frame == 0) {
				frame = 3;
			...
			if (dna_id > gene_len) {
				...
				if (codon_start == 1) {
/*
					fprintf(fp_out, "%d\t%d\t+\t%d\t%lf\t", start_t, end_t,
							frame, final_score);
